	...
	ylSlide.prototype = {
		...
		util: {
			eachs: function(ele, leng, Class, Create, pre, oneClass, addclass) { //渲染/解构dom
				...
				for(var i = 0; i < leng; i++) {
					...
					oneClass !== undefined && oneClass !== '' && i == 0 ? div.className = oneClass : (div.className = Class, addclass !== undefined && addclass !== '' ? div.className += ' ' + addclass + i : '');
					//代码片段中添加div
		...
		slide:{
			...
			addImgStyle:function(elements,ele,i,play){//添加图片动画style				
				elements.find('.'+$($(ele[i])[0]).attr('class')).css({'animation':'Yl-img_an '+parseInt(play/1000+1)+'s forwards'});
			},
			removeImgStyle:function(elements,ele){
				var x=Object.keys(elements.find(ele)).length;
				
					for(var k=0;k<x;k++){
						elements.find('.'+$($(ele[k])[0]).attr('class')).css({'animation':''});
					}
			},
			...
			fontImgActive:function(elements,util,imgTemplate,pic_index,stylePrefix,fontTemplate){
				...
				var p=Object.keys(fontTemplate).length,k=0,v=0;
				for(;k<p;k++){
		...
		render: function() {
			...
			if($(self.$element.selector).length > 0) {
				...
				self.decon(function(e) {
					...
					//是否创建分页层
					e.pages == true ?
						(self.$element.find('div').hasClass(self.util.splits(e.pagination)) == false ?
